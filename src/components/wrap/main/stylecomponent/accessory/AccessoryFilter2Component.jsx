...
export default function AccessoryFilter2Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("모자")
            || item.caption.includes("목걸이")
            || item.caption.includes("귀걸이")
            || item.caption.includes("반지")
            || item.caption.includes("보석")
            || item.caption.includes("jewerly")
            || item.caption.includes("ring")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/accessory/jewelry' className='filter-group__rapid-btn on'>주얼리</Link>
                    </label>
                    <label>
                    ...
                    <label>
                        ...
                        <Link to='/style/accessory/wallet' className='filter-group__rapid-btn'>지갑</Link>
                    </label>
                  </div>
              </div>
