...
export default function AccessoryFilter4Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("헤드폰")
            || item.caption.includes("head phone")
            || item.caption.includes("headphone")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/accessory/headphone' className='filter-group__rapid-btn on'>헤드폰</Link>
                    </label>
                    <label>
