...
export default function AccessoryFilter5Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("키링")
            || item.caption.includes("keyring")
            || item.caption.includes("key ring")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/accessory/keyring' className='filter-group__rapid-btn on'>키링</Link>
                    </label>
                    <label>
