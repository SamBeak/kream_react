...
import LuxuryFilterComponentChild from './LuxuryFilterComponentChild';

export default function LuxuryFilter1Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("루이비통")
            || item.caption.includes("lv")
            || item.caption.includes("louisvitton")
            || item.caption.includes("louis")
            || item.caption.includes("vitton")
          );
          setState({
            ...state,
            post: filteredData
            ...
            <div className="content">
              <nav className="tap-container">
                <ul className="tap-inner">
                  ...
                  {/* 럭셔리 */}
                  <li><Link to="/style/luxury" className='on'><span>럭셔리</span></Link></li>
                  {/* 의류 */}
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/luxury/lvmh' className='filter-group__rapid-btn on'>루이비통</Link>
                    </label>
                    <label>
                    ...
                    <label>
                        ...
                        <Link to='/style/luxury/rolex' className='filter-group__rapid-btn'>롤렉스</Link>
                    </label>
                  </div>
              </div>
              ...
              <LuxuryFilterComponentChild post={state.post} sorted={state.sorted} />
            </div>
          </div>
        </div>
      </article>
