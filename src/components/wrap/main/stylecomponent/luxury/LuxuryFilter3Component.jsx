...
export default function SneakersFilter3Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("에르메스")
            || item.caption.includes("Hermes")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/luxury/hermes' className='filter-group__rapid-btn on'>에르메스</Link>
                    </label>
                    <label>
