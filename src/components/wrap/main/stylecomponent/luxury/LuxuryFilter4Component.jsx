...
export default function SneakersFilter4Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("balenciaga")
            || item.caption.includes("발렌시아가")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/luxury/balenciaga' className='filter-group__rapid-btn on'>발렌시아가</Link>
                    </label>
                    <label>
