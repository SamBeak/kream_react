...
export default function ClothingFilter5Component() {
      ...
      .then((res) => {
        if (res.status === 200) {
          const filteredData = res.data.post.filter((item) =>
            item.caption.includes("스톤 아일랜드")
            || item.caption.includes("스톤아일랜드")
            || item.caption.includes("stone Island")
            || item.caption.includes("stone")
          );
          setState({
            ...state,
            post: filteredData
              ...
              <div className="filter-group__container">
                  <div className="filter-group__filter">
                    ...
                    <label>
                        ...
                        <Link to='/style/clothing/contemp' className='filter-group__rapid-btn'>컨템포러리</Link>
                    </label>
                    <label>
                    ...
                    <label>
                        ...
                        <Link to='/style/clothing/stone' className='filter-group__rapid-btn on'>스톤 아일랜드</Link>
                    </label>
                    <label>
